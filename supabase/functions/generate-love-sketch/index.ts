...
CRITICAL: This must look like an actual photograph taken by a professional photographer, not an artistic rendering. Focus on photographic realism, natural human features, and technical photography excellence.`;
    ...
    console.log(
      "Generating ultra-realistic image with prompt:",
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return new Response(
          JSON.stringify({
            error:
              "Too many requests to image API. Please try again in a few seconds.",
          }),
          {
            status: 429,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
...
Personality: ${personalityTraits}`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a mystical expert in astrology and love. Provide positive romantic and spiritual interpretations.",
            },
    ...
    const interpretation =
      interpretationData.choices[0]?.message?.content ||
      "A special connection awaits to be discovered through the stars.";
