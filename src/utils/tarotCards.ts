...
export const getCardInterpretation = (card: TarotCard, position: string, userProfile: any): string => {
  ...
  return `**${card.name}** ${card.isReversed ? '(Reversed)' : ''} in the position of **${position}**:\n\n${meaning}\n\n${personalizedContext}`;
};

const getPersonalizedContext = (userProfile: any): string => {
  if (!userProfile) return '';
  
  const contexts = [
    `As ${userProfile.goal || 'your spiritual journey'} is important to you, this card suggests reflection on your personal values.`,
    `Considering your current moment, this energy may bring valuable insights for your growth.`,
    `This card resonates with your quest for ${userProfile.goal || 'self-knowledge'} and may indicate the next step in your journey.`,
