...
export const TarotHistory: React.FC<TarotHistoryProps> = ({
  ...
  const getSpreadName = (
    ...
    return t(`tarot.history.spreadNames.${spreadMap[spreadType]}`);
  };
  ...
  if (isLoading) {
    return (
      <div className="text-center text-muted-foreground py-8">
        ...
        <p>{t("tarot.history.loadingReadings")}</p>
      </div>
    );
  }
  ...
  if (readings.length === 0) {
    return (
      <div className="text-center text-muted-foreground py-8">
        ...
        <p>{t("tarot.history.noReadings")}</p>
        <p className="text-sm">{t("tarot.history.noReadingsDesc")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {readings.map((reading) => (
        <Card
          key={reading.id}
          className="mystic-border hover:cosmic-glow transition-all duration-300"
        >
          <Collapsible
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${
                        spreadColors[reading.spreadType]
                      }`}
                    />
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">
                      {reading.cards.length} {t("tarot.deck.cards")}
                    </Badge>
              ...
              <CardContent className="pt-0">
                    ...
                    {reading.cards.map((card, index) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-muted-foreground mb-2">
                          {card.suit === "major"
                            ? t("tarot.history.majorArcana")
                            : `${card.suit}`}
                        </div>
                        {card.isReversed && (
                          <Badge variant="outline" className="text-xs">
                            {t("tarot.history.reversed")}
                          </Badge>
                        )}
                      </div>
                    ))}
                ...
                <div>
                  <h4 className="font-semibold mb-3">
                    {t("tarot.history.interpretation")}
                  </h4>
                      ...
                      {reading.interpretation.split("\n").map((line, index) => {
                        if (line.startsWith("##")) {
                          return (
                            <h3
                              key={index}
                              className="text-base font-playfair text-primary mt-3 mb-2"
                            >
                              {line.replace("##", "").trim()}
                            </h3>
                          );
                  ...
                  <div className="flex justify-end mt-4 pt-4 border-t">
                    ...
                    >
                      ...
                      {deletingId === reading.id
                        ? t("tarot.history.deleting")
                        : t("tarot.history.delete")}
                    </Button>
                  </div>
                )}
              </CardContent>
