...
const SpiritualDiary = () => {
    ...
    const fetchEntries = async () => {
      try {
        ...
        toast({
          ...
          description: t("spiritualDiary.errorLoading"),
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const entryTypes = {
    dream: {
      icon: Moon,
      label: t("spiritualDiary.types.dream"),
      color: "bg-blue-500",
    },
    feeling: {
      icon: Heart,
      label: t("spiritualDiary.types.feeling"),
      color: "bg-pink-500",
    },
    insight: {
      icon: Lightbulb,
      label: t("spiritualDiary.types.insight"),
      color: "bg-yellow-500",
    },
    experience: {
      icon: Sparkles,
      label: t("spiritualDiary.types.experience"),
      color: "bg-purple-500",
    },
  };

  const handleAddEntry = async () => {
    if (!newEntry.title.trim() || !newEntry.content.trim() || !user || !profile)
      return;
    ...
    try {
      ...
      toast({
        title: t("spiritualDiary.entrySaved"),
        description: t("spiritualDiary.entrySavedDesc"),
      });
    } catch (error) {
      console.error("Error saving diary entry:", error);
      toast({
        title: t("common.error"),
        description: t("spiritualDiary.errorSaving"),
        variant: "destructive",
      });
    } finally {
      setSaving(false);
  ...
  const handleDeleteEntry = async (entryId: string) => {
    try {
      ...
      toast({
        title: t("spiritualDiary.entryDeleted"),
        description: t("spiritualDiary.entryDeletedDesc"),
      });
    } catch (error) {
      console.error("Error deleting entry:", error);
      toast({
        title: t("common.error"),
        description: t("spiritualDiary.errorDeleting"),
        variant: "destructive",
      });
    }
  };
              ...
              <CardContent className="space-y-4">
                <div>
                  <label className="text-sm font-medium mb-2 block">
                    {t("spiritualDiary.entryTypeLabel")}
                  </label>
                    ...
                    {Object.entries(entryTypes).map(([key, type]) => {
                      ...
                      return (
                        <Button
                          key={key}
                          variant={
                            newEntry.type === key ? "cosmic" : "ethereal"
                          }
                          onClick={() =>
                            setNewEntry((prev) => ({
                              ...prev,
                              type: key as DiaryEntry["type"],
                            }))
                          }
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block">
                    {t("spiritualDiary.descriptionLabel")}
                  </label>
                  <Textarea
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {saving
                      ? t("spiritualDiary.saving")
                      : t("spiritualDiary.saveEntry")}
                  </Button>
                  <Button
                    variant="ethereal"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {formatDate(new Date(entry.created_at), language)}{" "}
                            {t("spiritualDiary.at")}{" "}
                            {formatTime(new Date(entry.created_at), language, {
                              hour: "2-digit",
                              minute: "2-digit",
                            })}
                          </p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              window.confirm(
                                t("spiritualDiary.deleteConfirmation")
                              )
                            ) {
